...
static void _s_init_v(void)
{
    ...
    PCC_LC_ZC_Init_v(&_s_zc_control_s, ZC_LC_ZC_SENSITIVE_TO_ANY_EDGE_d);
}
...
static void _s_start_pulses_v(void)
{
    ...
    SET_BIT(TIM1->CCER, TIM_CCER_CC1E | TIM_CCER_CC2E);                     /* Enable compare channel 1 and 2. */
...
static void _s_inhibit_pulses_v(void)
{
    ...
    CLEAR_BIT(TIM1->CCER, TIM_CCER_CC1E | TIM_CCER_CC2E);                   /* Disable compare channel 1 and 2. */
    CLEAR_BIT(TIM1->CCMR1,TIM_CCMR1_OC1PE | TIM_CCMR1_OC2PE);               /* Disable pre-load of OC1 and OC2. */
}
